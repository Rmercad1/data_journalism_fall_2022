...
**Answer** It looks like the Tabulizer command took the tables and made them into a dataset. However, it doesn't look like its complete or that there are some numbers missing. It does not have the county info. 
...
**Answer** From the original tabulizer set, we know have two sets that are much cleaner than the last one. Its still a little confusing for me, partly because I can't find the removals for some but I get what the two new tables are presenting, which is mew registrations amalgamated. 
Edit: Oh we haven't made the removals one yet, nevermind!
...
**Task** Run the following code and describe the result.
**Answer** 
Now we have the second data set, the removals and I can see the tibble that was created looks much nicer, easier to read and males sense. It can now be analysed. Much better than the original set we loaded in. The oddly named columns with "Xs" threw me off. 
```{r}
# make the new dataframe, renaming the columns and ditching the columns we don't want
...
**Answer**
At first glance, I'm inclined to say there is no one category where republicans are on par with democrats, percentage wise, but this doesn't mean much when more Democrats registered than Republicans. It still looks like Republicans are more inclinced to register in more "traditional" methods, such as in-person or at the MVA. What truck out to me was "Eric Report" which I looked up and found it had to so with education. Are these students who registered at their high schools before their 18th birthday? That is how I registered to vote in MD.
...
**Task** Run the following code and describe what you think are the most interesting percentages in the dataframe.
**Answer** Again, not but percentages for Republicans but that is because they're a smaller population than Democrats. What sticks out to me is MORE UA voters moved counties than Republicans and about the same about of UA and Republican voters moved out of state or had duplicates. 
...
**Task** Run the following code. What's the name of the new dataframe? Its called voter_reg_page_1
...
**Answer** The original ones for June and October had 17 rows and the combined one has 16, the absnetee row is messed up. 
...
**Task** Run the following code. How many rows do you have compared to either the October or June dataframes?
**Answer** The dataframe now has 17 rows, just like the original dataframes it takes from and the absentee row is now visible. 
...
**Answer** From first glance there are way too many columns, its hard to read especially when you are scrolling the the right and need to scroll back left to remember the name of the row. Its basically three data frames put together, side by side. It is readable and the data can be interviewed, but its a very big. 
...
**Answer** First off, genius to make a chart, this makes things much more readable from the big table we just created.
The trends in the data indicate that Democrats will mostly rely on ERIC (school registration if I'm correct), Absentee, Volunteer, and provisional ballot for registrations. Mail is the most popular for Democrats who relied heavily on it during the pandemic. 
...
**Answer** The top methods have changed! For October it is provisional ballot while in October ir is ERIC. This makes sense since in June, school is partly out of session while in October, it is in session and there are probably advanced efforts in schools to register students to vote by the time they turn 18. 
