...
**Answer** It creates a tibble, a very large tibble with columns with one candidate and how many votes that got in each county on election day. It was created with mutate.
...
**Answer** It gives a range if data. It shows the percentage that candidates recieved on election day instead of from other voting methods.
...
**Answer** Somewhat, it basically now shows candidates in what percentage of their votes were from election day but by election day only. It could indicate which candidates really did a mail-in ballot campaign perhaps? it does show that it is mostly Democrats. This will allow candidates to focus on target groups for specific voting campaigns for voting in-person, or by mail or another method. 
...
str_to_title: Makes whatever the function is into a title. Ex: The brown wooden door becomes: "The Brown Wooden Door."

str_to_sentence:Makes whatever the function is into a sentence.
...
**Answer** It is ordering the data set into the amounts listed below.
...
**Answer** Because this new variable is now filtered to what we want? I'm not entirely sure but I assume it has to do with that is being filtered. It is named "Selected" so I guess it means that the new filtered data is now in a function that indicated it as such. 
...
**Answer:** It puts the dates in a numeral format that is easier to read and ensures it is the correct date. 
...
**Answer** I think what happened was that the code grouped all the expidentures for a single month into one specific day, in this case, the first of the month. 
