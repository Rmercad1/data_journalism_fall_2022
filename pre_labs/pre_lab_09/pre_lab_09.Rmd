...
Go to the console and install it with 
...
**Answer**
It is showing me detailed variables about each precinct with extremely precise shape lengths. Other things such as precinct number and others are also available as well.It doesn't show me location though. 
```{r}
View(pg_precincts)
```
...
**Answer**
Now that we have geographical coordinates, the information we have can now be plotted on a map to show us precise boundaries of crecints, which is below. 
```{r}
pg_precincts %>%
  ggplot() +
  geom_sf() +
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
It loads tigris, which pulls shape and line data from the US census. 
```{r}
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** It loads data from all 3,234 US counties from tigris. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The large data set was filtered out so only the Maryland counties will show in the set now. 
```{r}
...
We know have a big beautiful map of Maryland based on county boundaries, interestingly it doesnt show the Bay. 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The map's layers have been filtered yet again to PG county with all the precinct layers now added to the map, we now have a nice beayitufl map of PG county with its precincts. However, I do see the layers on some of the boundaries. 
...
**Task** Run the following code. Describe the output in the space below.
**Answer** 
We have loaded a new data frame that now gives us information about voter turnout based on precincts throughout the state. We then took that and filtered out the results to show us PG turnout. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
You joined together the two variabes to now have one with PG precincts, their turnours, and also shape and other info. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
You used ggplot to map out the dataframe you created. The colors show precincts by how many elibidle voters there are. 
```{r}
ggplot() +
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
A new dataframe was loaded which shoes registered voters per county by party affiliation. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
You got the population of each county in MD from the tidy census. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
You got geographic infro from teh tidy cesnsus for PG county. 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The data set is now nicely cleaned up so that we can use the set in better way. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
You joined the two data sets md population and the votes per 10K so that we can now see voting data for each county. 
```{r}
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
We cleaned up the names of Saint Marys and Baltimore County so that they no longer show NAs. 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
Two data sets are left joined so that we can calculate number of votes per 10,000 populatiln for the counties and see them in the dataframe. 
```{r}
md_voters_per_10k <- md_county_population %>%
  left_join(voters_by_county) %>%
...
We put the dataframe into a map. We now can see counties with voters per 10k. 
...
**Task** Run the following code. Describe the output in the space below.
**Answer**
The colour was changed so that now we can better see variations in data with new colours. 
```{r}
ggplot() +
  geom_sf(data=md_voters_per_10k, aes(fill=voters_per_10k)) +
  theme_minimal() +
...
**Answer**
You changed the colour patterns to log? I am not entirely sure what you did here. 
```{r}
ggplot() +
  geom_sf(data=md_voters_per_10k, aes(fill=voters_per_10k)) +
  theme_minimal() +
...
**Task** Run the following code. Describe the output in the space below.
**Answer** 
Datasets were joined to see what we just did for unaffiliated voters to see them per 10000 per the population. 
...
**Answer** 
We mapped the data for UA voters with a good colour scheme to get the results. 
```{r}
ggplot() +
