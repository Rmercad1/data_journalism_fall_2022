...
**Answer** It is not possible for someone who registered in 1900 to still be alive, same with the second one. The first 20 results should be ok, but ultimately, as people get elderly, they die, as we know, and updates to voter registration have to be made. Its possible that the two 1900 people are a mistake in the system. 
```{r}
yadkin_voters %>% 
...
**Answer** Confirmation not returned and confirmation pending is alarming. As we said in class, Texas is one of the few states where you can be kicked off voter rules. Or maybe that was NC? Either way, these things make registering to vote hard for people who move around a state a lot since the state needs to keep track of what they're doing. 
...
**Answer** It is the amount of money a person has given through a single contribution. The aggregate amount next to it gives you the total amount of their contributions over a period. Not just one.
```{r}
glimpse(conowingo)
```
...
**Answer**
1) Vote is in character amount.
2) Mispeliings of names, ex: St, street; ln, lane; confuses R and causes it to group those as separate things when they shouldn't be. 
3) Conoringo is capitalised in one instance and not in a second, which also confuses R and causes the sa,e problems as the previous answer.
4) Zip code is listed as a character. 
5) Column names are not uniform, some are capitalised, others are not, etc. 
```{r}
conowingo
```
...
**Answer**
It puts an "X" infront of it, in the string, you do this because you can't have a row title start with a number or a regular character. 
...
Answer: The name of the column no longer has that weird title anymore. 
```{r}
cleaned_conowingo <- conowingo %>%
  clean_names() %>% 
...
**Answer** It has something to do with dealing with the two sets: total_conowingo, and cleaned_conowingo. Perhaps about the math that goes intoit?
...
**Answer** There are two rows that are duplicates, about a man named Derrick Hamilton.
...
**Answer** It reduced the rows to 21, getting rid of the duplicate row. 
```{r}
# cleaning function
cleaned_conowingo <- conowingo %>%
  clean_names() %>% 
...
**Answer**
 The Zip code is now a regular 5-digit zip code that we all understand. 
```{r}
# cleaning function
cleaned_conowingo <- conowingo %>%
  clean_names() %>% 
...
Answer: There is only one mispelling of Conowingo.
```{r}
# cleaning function
cleaned_conowingo <- conowingo %>%
  clean_names() %>% 
