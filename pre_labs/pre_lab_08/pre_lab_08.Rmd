...
**Answer**
First line lodes the data and calls it a new variable, second line arrange(descby the total difference in numbers.
Third lines selects the number we want to show and the way we want it to show.
Finally we do the head for the data to limit it when we view it.
And then we finally show the data. 
...
**Answer**
There is a blank space, because we need to indicate how we want the data to be arranged in the chart.
```{r}
maryland_top_counties %>%
  ggplot()
```
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer**
We have now plotted the data properly indicating what we want on the X axis and the Y axis plus our labels. But it looks very messy. 
```{r}
maryland_top_counties %>%
  ggplot() +
...
**Answer**
It re-ordered the counties using the total difference. Now is it ordered from least to greatest. 
```{r}
maryland_top_counties %>%
  ggplot() +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** The function switched axises of the variables to better fit the names. Now the county name is on the Y axis and you can clearly see the county names. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** The background of the graoh is now a nice white, it is not grey. Aesthicaically is is much nicer and clearer to look at. 
...
**Answer**
It says that ggthemes was installed. 
```{r}
...
**Answer** It looks like a chart published in the economist. It has blue on its background. 
```{r}
maryland_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(County,TOTAL_DIFF), weight=TOTAL_DIFF)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** The graph looks much better and clearer. There is now a title, the XA and Y axix labels are now clearer and correctly spelled. It's an ok chart, still needs some work though. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** It shows total contributiosn for the date listed on the left od it. So 580 contributions on April 1, 2022. I just don;'t know to whom these contributions are going. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
It shows different contributions over time as it ebs and flows. It shows a big big spike in June as the primary election was nearing and financial reporting deadlines are due. 
```{r}
md_winred_by_date %>%
  ggplot() + 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
The x axis dates are now more spread out over different dates in the months instead oj just by month. It is now ordered by diffeny weeks. We can now pinpoint which weeks within months saw these ebbs and flows.  
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer**
It turned the date labels in the X axis 45 degrees to the left and it is now better to see and read. The turning makes the dates not looked smushed together and the graph looks cleaner now. 
