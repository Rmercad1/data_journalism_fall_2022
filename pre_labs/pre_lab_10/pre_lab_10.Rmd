...
Visualization : https://datawrapper.dwcdn.net/WdM8b/1/
Sharing : https://www.datawrapper.de/_/WdM8b/
...
md_gov_county <-md_gov_county%>%
  mutate(total_votes = Moore+Cox+Lashar+Wallace+Harding+`Write-ins`*100) %>% 
...
The code that we created only has Maryland data, no other counties have data, therefore we don't need the rest of the nation, so datawrapper took away those counties with no data, focusing on MD. 
...
The code for the county was taken away and we were given two new options. Now we can change the maps so that when a person hovers over the county, we can see the county name and now a nice percentage for the county. 
...
For sharing: https://www.datawrapper.de/_/jgGj3/
Visualization: https://datawrapper.dwcdn.net/jgGj3/1/
