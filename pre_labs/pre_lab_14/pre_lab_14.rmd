...
It takes all the press releases from our source and puts them into a data frame. The column names are: date, title, URL, text.
...
It uniformalizes the variations in the spelling of the words so that when you look up the word "dog" it will show up with all its variations.  
...
**Answer**
Basically, ALL the punctuation in all of the memos is now treated as disinct words, which can be overbearing since there are nearly 600K words. 
...
**Answer**
The words we found from the previous code are now grouped together by ggplot showing which ones show up the most. However, as stated, it is EVERY single piece of punctuation so redundant words like "the" or "a" show up a lot. 
...
**Answer**
Now that we've cleared out the stop words, the results are much more interesting. We can see that certain key words like "health" and "funding" and "covid" were very prevalent in the memos. While the percentage is small for these words, that shouldn't matter considering the sheer number of words that are in all the memos. 
```{r}
unique_words %>%
...
**Answer**
You can definetly see a shift away from the pandemic in the responses from 2021 to 2022. The top 10 list for '21 has more health focused and Covid-19 driven, whereas in '22, the list is more "normal" you could say, or focuses on much more different topics aside from health and more on economic recovery. 
```{r}
unique_words_2021 %>%
  anti_join(stop_words) %>%
  group_by(word) %>%
...
**Task** Run the following code and describe the results.
**Answer**
Instead of just single words, now we have the power to look for phrases using n anb bigrams, which can come in handy when you want to parse through memos or tweets for specific phrases. Here was can see in 2021 that, naturally, covid-19 was the largest "phrase" since the disease name has a slash and number connected to a word. Other important things include the American Rescue plan act passed by Congress, the bay, and some names of politicians from Maryland. 
```{r}
releases %>%
  filter(date < '2022-01-01') %>%
...
**Task** Run the following code and describe the results.
**Answer**
As with the previous top 10 list for '22, we actually see here that in terms of phrases, the pandemic was still very much discussed throughout the memo.. at leasy in the top results of instances and percentages. Other than that topic, "normal" topics you could say dominate such as projects, the bay, Baltimore, PG, etc. Checks out for a Senator from MD.
```{r}
releases %>%
  filter(date >= '2022-01-01') %>%
  unnest_tokens(bigram, text, token = "ngrams", n = 2) %>%
...
**Answer**
I see words like "oversight" and "confront" are listed as "negative" when really, the context of those words are important for nuance. It's really strange to group words into just two binary options as they can all have multiple meanings. Same thing with words that are listed as "positive." "Rich" and "revolutionary" are listed as positive but those two words could have negative contexts such as the rich not paying their fair share of taxes, or revolutionary activity in a country is causing instability. Context is key here.
```{r}
