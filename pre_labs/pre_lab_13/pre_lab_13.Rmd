...
**Answer** 
When r extracts the tables from the html of the url, I assume that the column headers come out as "1", "2", etc, and not the names we want. So, when you save in the table as a variable, you have to basically do what we do for every data set, clean it up. So we clean up the names for the columns, and set them as a number. 2, 3, and 4 refer to the number position of the columns. Column 2 becomes Peroutka and so on.
...
**Answer** 
It means that there are up to 20 different unordered lists in this HTML source and that within these lists are many li tags or lists of there own. 
...
**Answer** 
From what I see, it looks like the li tags turned into names of elements that you would find on the website. It resembles headers or menu buttons that you would find on websites. Or they could resemble drop-down menus. In this case, they're committee names. 
...
**Answer** 
Basically it is just the li elements, turned into text, and then ordered into a tibble. Though it is taking all the text and ordering them separately. Though towards the end it looks like those row names don't belong in the data frame. 
